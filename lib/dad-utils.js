...
module.exports = {
    ...
    readHDKey : function(did) {
        ...
        let config = this.readConfig();
        var file = config.workingPath + "/" +did.replace(/:/g, '_')+".json";
    ...
    getDID : function(did) {
        ...
        var document = did.replace(/:/g, '_')+".json";
    ...
    getDAD : function(did,chain) {
        ...
        var document = ddid.replace(/:/g, '_')+".json";
        var file = config.workingPath + "/" +document;
        if(fs.existsSync(file))
        {
            console.log("Cannot create '"+document+"'. It already exists.");
    ...
    verifyDAD : function(did,chain) {
        ...
        let config = this.readConfig();
        var file = config.workingPath + "/" +ddid.replace(/:/g, '_')+".json";
        if(!fs.existsSync(file))
        {
            console.log("File '"+file+"' not exists.");
            return;
    ...
    sendFileEncrypted(file,chain,targetDid) {
        ...
        input.pipe(cipher).pipe(output).on("close",() => {
                ...
                var fileDad = config.workingPath+"/"+ddid.replace(/:/g, '_')+".json";
